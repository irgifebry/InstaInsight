...
export const getBookmarkletHref = () => {
  const minifiedScript = `(async()=>{const sleep=ms=>new Promise(r=>setTimeout(r,ms));const getCookie=name=>{const v="; "+document.cookie;const p=v.split("; "+name+"=");return p.length===2?p.pop().split(";").shift():null};const uid=getCookie("ds_user_id");if(!uid)return alert("Please login to Instagram first!");const div=document.createElement('div');div.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;z-index:999999;background:rgba(9,11,17,0.96);color:#f8fafc;display:flex;align-items:center;justify-content:center;font-family:sans-serif;";div.innerHTML=\`<div style="background:#131824;border:3px solid #f8fafc;padding:24px;width:300px;text-align:center;box-shadow:6px 6px 0 #000;border-radius:12px;"><h3 id="st" style="margin:0 0 10px;font-size:16px;">Starting...</h3></div>\`;document.body.appendChild(div);const ids=["936619543551","1217981644879628"];try{const m=document.body.innerHTML.match(/"app_id":"(\\\d+)"/);if(m)ids.unshift(m[1])}catch(e){}let cur=0;async function get(type){let list=[],maxId="",hasNext=true;while(hasNext){document.getElementById("st").innerText=\`Fetching \${type}... (\${list.length})\`;try{const r=await fetch(\`https://www.instagram.com/api/v1/friendships/\${uid}/\${type}/?count=200&max_id=\${maxId}\`,{headers:{"x-ig-app-id":ids[cur],"x-requested-with":"XMLHttpRequest","x-csrftoken":getCookie("csrftoken")}});if(r.status===429){await sleep(60000);continue}if(r.status===400){cur=(cur+1)%ids.length;await sleep(1000);continue}if(!r.ok){await sleep(2000);continue}const j=await r.json();list.push(...(j.users||[]).map(x=>({username:x.username})));maxId=j.next_max_id;hasNext=!!maxId;await sleep(800)}catch(err){await sleep(1000)}}return list}const flw=await get("followers");const flg=await get("following");const res={followers:flw.map(x=>({string_list_data:[{value:x.username}]})),following:flg.map(x=>({string_list_data:[{value:x.username}]}))};const json=JSON.stringify(res);div.innerHTML=\`<div style="background:#131824;border:3px solid #f8fafc;padding:24px;width:300px;text-align:center;box-shadow:6px 6px 0 #000;border-radius:12px;"><h3 style="margin:0 0 10px;font-size:16px;">SUCCESS!</h3><textarea id="ta" readonly style="width:100%;height:80px;background:#090b11;color:#f8fafc;border:2px solid #f8fafc;font-family:monospace;font-size:10px;padding:8px;box-sizing:border-box;resize:none;border-radius:8px;">\${json}</textarea><button id="cb" style="width:100%;margin-top:12px;padding:10px;background:#e2e8f0;color:#090b11;border:2px solid #f8fafc;font-weight:bold;cursor:pointer;border-radius:8px;box-shadow:3px 3px 0 #000;">COPY RESULTS</button></div>\`;const cb=document.getElementById("cb");const ta=document.getElementById("ta");cb.onclick=()=>{ta.select();document.execCommand("copy");cb.innerText="COPIED!";cb.style.background="#22c55e";cb.style.color="#fff";alert("Data copied! Return to InstaInsight tab.")};if(window.opener){window.opener.postMessage({type:"IG_DATA_SYNC",data:res},"*")}})();`;
